...
int getWinner(int m, const vector<int>& profile, const vector<double>& weights) {
    ...
    for (int outcome = 0; outcome < num_outcomes; ++outcome) {
        ...
        } else if (abs(score - min_score) <= eps) {
...
vector<double> generateFiWeights(int n, int i_zeros) {
...
bool isManipulable(int m, const vector<int>& profile, const vector<double>& weights) {
    int n = profile.size();
...
int countManipulableChunk(int m, const vector<vector<int>>& sampled_elections, 
                          const vector<double>& weights, int start_idx, int end_idx) {
    int local_count = 0;
...
int main() {
    ...
    for (int m : {3, 4, 5}) {
        ...
        for (int i_zeros = 0; i_zeros <= n - 1; ++i_zeros) {
            vector<double> weights = generateFiWeights(n, i_zeros);
            ...
            for (unsigned int t = 0; t < num_threads; ++t) {
                ...
                futures.push_back(async(launch::async, countManipulableChunk, 
                                        m, ref(sampled_elections), ref(weights), start_idx, end_idx));
            }
